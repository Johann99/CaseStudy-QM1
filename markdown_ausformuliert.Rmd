...
#1.Filtern nach Personen, welche Bio als wichtig ansehen und weclhe Bio als unwichtig ansehen.
 ...
 GesamtzProzente<-cbind(Prozentfrauenunwichtig,Prozentfrauenwichtig,ProzentmännerUnwichtig,Prozentmännerwichtig)
...
Hierbei errechnen wir die Mittelwerte der Aussagen von Frage 3 und vergleichen diese in einem Säulendiagramm. Zudem berechnen wir auch noch die einzelnen Modalwerte und stellen auch diese in einem Säulendiagramm dar. 
```{r}
# Neue Matrix
...
Diese vergleichbaren Daten werden nun zusammengeführt und in einem Säulendiagramm miteinander verglichen.
...
#t.test Genuss
if(t.test(marke_robinson_dt$V11,marke_bio_dt$V12)$p.value < 0.05) {print(TRUE)}
...
Außerdem kann auch Frage 18 und Aussage 4 ("Die Marke ROBINSON steht für hohe Qualität, da bedarf es keiner weiteren Siegel") zur Prüfung der Hypothese herangezogen werden.  
Zunächst stellen wir das Ergebnis der Aussage 4 in einem Histogramm dar. Außerdem kann dann noch ein einseitiger T-Test durchgeführt werden, bei dem geschaut wird, ob der Mittelwert der Aussage 4 signifikant vom Wert 4 (= neutral) abweicht. Dabei legen wir zudem fest, dass der Mittelwert signifikant nach unten abweichen müsste, damit wir unsere Alternativhypothese annehmen könnten und ein signifikanter Bedarf an Gütesiegeln vorliegt.  
Nullhypothese:       H0 gleich 4
Alternativhypothese: H1 ungleich 4
